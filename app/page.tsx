  ...
  return <>
    ...
    <section className="hero">
      <div className="shell hero-grid">
        <div className="hero-copy"><span className="eyebrow"><span className="pulse" /> 대한민국 소상공인의 지원사업 실행 파트너</span><h1>지원사업 선정 이후,<br /><em>매출이 시작됩니다</em></h1><p>복잡한 지원사업은 쉽게 이해하고,<br className="desktop-only" /> 선정 이후 마케팅은 제대로 실행할 수 있도록 함께합니다.</p><div className="button-row"><Link className="button button-primary" href="/programs">내게 맞는 지원사업 찾기 <span aria-hidden="true">→</span></Link><Link className="button button-outline-coral" href="/after-selection#consult">선정 후 마케팅 실행 상담</Link></div><div className="hero-proof"><span><b>01</b> 지원사업 이해</span><span><b>02</b> 전문가 연결</span><span><b>03</b> 마케팅 실행</span></div><p className="hero-disclaimer">선정 여부는 주관기관의 심사 기준과 판단에 따르며, 선정을 보장하지 않습니다.</p></div>
        <div className="hero-visual" aria-label="한국인 소상공인 대표와 마케팅 전문가의 상담 장면"><img src="/images/korean-business-consulting-hero.png" alt="한국인 소상공인 대표와 마케팅 전문가가 노트북을 보며 상담하는 모습" /><div className="hero-float"><span className="float-icon">✓</span><div><strong>선정 후 실행 준비</strong><small>전략 · 제작 · 운영 · 결과보고</small></div></div><div className="hero-diagnosis"><strong>무료 자가진단</strong><p>5가지 질문으로 내 상황에 맞는 다음 단계를 확인하세요.</p><Link href="/diagnosis">지금 진단 시작하기 <span aria-hidden="true">→</span></Link></div></div>
      </div>
    </section>
    ...
    <section className="section"><div className="shell"><div className="section-heading split"><div><span className="eyebrow">실행 사례</span><h2>업종과 상황에 맞춘<br />실행 설계 예시</h2></div><div><p>아래 내용은 서비스 이해를 돕기 위한 샘플 사례이며, 실제 매출 성과나 지원사업 선정 결과를 의미하지 않습니다.</p><Link className="text-link" href="/cases">전체 사례 보기 →</Link></div></div><div className="case-grid">{cases.map((item) => <article className="case-card" key={item.slug}><div className="case-top"><span>{item.industry}</span><b>{item.status}</b></div><h3>{item.title}</h3><dl><div><dt>지원사업</dt><dd>{item.program}</dd></div><div><dt>고객 문제</dt><dd>{item.problem}</dd></div><div><dt>실행 서비스</dt><dd>{item.execution.join(" · ")}</dd></div></dl><Link href={`/cases/${item.slug}`}>상세보기 <span aria-hidden="true">→</span></Link></article>)}</div></div></section>

    <section className="section expert-section"><div className="shell"><div className="section-heading centered"><span className="eyebrow">전문가 소개</span><h2>사업과 마케팅을 함께 보는 사람들</h2><p>지원사업의 목적과 실제 고객 유입을 함께 이해하는 팀이 실행합니다.</p></div><div className="expert-grid">{experts.map((expert) => <article key={expert.name}><div className="avatar" aria-hidden="true">{expert.initials}</div><span>{expert.role}</span><h3>{expert.name}</h3><p>{expert.bio}</p></article>)}</div><div className="center-action"><Link className="button button-ghost" href="/experts">전문가 소개 자세히 보기 →</Link></div></div></section>
